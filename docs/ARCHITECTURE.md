...
Sub-components like `Fee` exist only as embedded fields within other messages. They have no independent lifecycle and therefore carry no envelope.
...
3. **Is it a sub-component?** If the message is only ever embedded inside another message (like `Fee` inside `FillEvent`), do NOT add an envelope. Sub-components inherit routing context from their parent.
