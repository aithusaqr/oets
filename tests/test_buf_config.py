...
def test_requirements_txt_pins_protobuf(requirements_txt: str):
    """requirements.txt must pin protobuf with a lower bound that matches the
    committed gencode version and an upper bound <6.

    The protobuf Python runtime rejects `runtime.minor < gencode.minor`. The
    committed `_pb2.py` files in `generated/python/` declare their gencode
    version in a header comment (`# Protobuf Python Version: X.Y.Z`); the pin
    in requirements.txt must be compatible with that gencode.

    For now we accept any `>=5.NN,<6` where NN matches the lowest gencode
    minor among committed pb2 files — the actual pin will tighten when pb2s
    are regenerated against newer gencode.
    """
    import re
    ...
    pin_match = re.search(r"protobuf\s*>=\s*5\.(\d+)\s*,\s*<\s*6", spec)
    assert pin_match is not None, (
        f"requirements.txt protobuf pin must be of the form 'protobuf>=5.NN,<6'; got {spec!r}"
    )
